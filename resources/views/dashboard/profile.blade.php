...
@section('content')
    ...
    <div class="flex justify-center mb-6 space-x-4">
        <button data-modal-toggle="editPhoto"
            class="btn btn-info flex items-center space-x-2 px-4 py-2 text-white bg-blue-500 rounded">
            <i class="fa fa-edit"></i><span>Edit Photo</span>
        </button>
        <button data-modal-toggle="resetPass"
            class="btn btn-danger flex items-center space-x-2 px-4 py-2 text-white bg-red-500 rounded">
    ...
    <div class="max-w-xl mx-auto bg-white dark:bg-gray-800 shadow rounded-lg">
        <div class="p-6 text-center">
            ...
            <img src="{{ $user->foto ? asset('images/profile/' . $user->foto) : asset('images/default.png') }}"
                alt="Profile Photo" class="w-32 h-32 rounded-full mx-auto mb-4">
            @if (session('success'))
            ...
            <div class="text-left space-y-4">
                ...
                <div>
                    ...
                    <span class="text-gray-700">{{ $pegawai->jabatan->nama_jabatan }}</span>
                </div>
                <div>
                    <span class="font-semibold">Golongan:</span>
                    <span class="text-gray-700">{{ $pegawai->golongan->nama_golongan }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="editPhoto" class="hidden fixed inset-0 z-50 flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-96 p-6">
            ...
            <form method="POST" enctype="multipart/form-data" action="{{ route('dashboard.profile.update-photo') }}">
                @csrf
                <input type="hidden" name="nip" value="{{ $pegawai->nip }}">
                <input type="file" name="gambar" required
                    class="mb-4 w-full text-sm text-gray-600 file:py-2 file:px-4 file:border file:rounded file:text-sm file:font-semibold
    ...
    <div id="resetPass" class="hidden fixed inset-0 z-50 flex items-center justify-center">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-96 p-6">
            ...
            <form method="POST" action="{{ route('dashboard.profile.reset-password') }}">
                @csrf
                <input type="hidden" name="nip" value="{{ $pegawai->nip }}">
                <div class="mb-4">
